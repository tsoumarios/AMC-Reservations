...
if (document.querySelectorAll('.confeti').length > 0) {
  // Confetto Class
  var Confetto = function Confetto() {
  ...
  var initBurst = function initBurst() {
  ...
  var textElements = button.querySelectorAll('.button-text');
  textElements.forEach(function (element) {
    var characters = element.innerText.split('');
    var characterHTML = '';
    characters.forEach(function (letter, index) {
      characterHTML += "<span class=\"char".concat(index, "\" style=\"--d:").concat(index * 30, "ms; --dr:").concat((characters.length - index - 1) * 30, "ms;\">").concat(letter, "</span>");
    });
